...
class Runtime(types.ModuleType):
  ...
  @type tid:			C{int}
  @param tid:			Thread identifier, as returned by L{thread.get_ident}
  @type stats:			C{dict}
  @param stats:			Shortcut for L{hlib.stats.stats}.
  @type user:			C{hlib.datalayer.User}
  @param user:			Current user if known, C{None} otherwise.
  @type db:			L{hlib.database.DB}
  @param db:			Database object as passed to application.
  @type dbconn:			C{None}
  @param dbconn:		Current database connection. No need to use it.
  @type dbroot:			L{hlib.datalayer.Root}
  @param dbroot:		Database root - root of object tree stored in ZODB.
  @type root:			L{hlib.handlers.GenericHandler}
  @param root:			Handler root - root of object tree of handlers.
  @type server:			L{hlib.datalayer.Server}
  @param server:		Server-specific data stored in database.
  @type dont_commit:		C{bool}
  @param dont_commit:		If set to C{True} current transaction will be rolled back. Set to C{True} in case of error or when executing read-only method.
  @type ui_form:		C{string}
  @param ui_form:		ID of HTML form that is being generated by four templating library (C{w_form_*} functions)
  @type app:			L{hlib.engine.Application}
  @param app:			Application current request belongs to.
  ...
  @param session:		Initialized Session object, with all stored data loaded, if there is session available. C{None} otherwise.
  @type time:			C{int}
  #param time:
